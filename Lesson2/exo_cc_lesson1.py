...
def pig_latin(text):
    words = text.split()
    return ' '.join([word[1] + word[2:] + word[0].lower() + 'ay' for word in words]).capitalize()
...
def extract_data(data):
    cities = []
    for city, infos in data.items():
        cities.append({'name': city,
                       'country': infos['metadata']['country'],
                       'date': infos['weather_list'][0]['dt_txt'],
                       'temp': int(infos['weather_list'][0]['main']['temp']-273.15)})
    return cities
